...
from src.config import IMAGE_DIR, TOP_K_RECOMMENDATIONS  # noqa: E402
...
def resolve_article_image_path(article_info: dict) -> Path | None:
    """Find a local H&M article image if the raw image folder is available."""
    ...
    candidates = [
        IMAGE_DIR / padded_article_id[:3] / f"{padded_article_id}.jpg",
        IMAGE_DIR / f"{padded_article_id}.jpg",
    ]
...
def image_to_data_uri(image_path: Path | None) -> str | None:
    """Convert a local image file to a data URI for HTML cards."""
...
def render_product_cards(recommendations, articles_df: pl.DataFrame) -> None:
    ...
    for rank, (item_id, score) in enumerate(recommendations, 1):
        ...
        image_uri = image_to_data_uri(resolve_article_image_path(article_info))
        image_html = (
            f'<img class="product-img" src="{image_uri}" alt="{html.escape(str(product_name))}">'
            if image_uri
            else '<div class="img-placeholder">No Image</div>'
        )
        ...
        <div class="product-card">
            ...
            <div class="product-name">{html.escape(str(product_name))}</div>
            <div class="product-meta">
                <b>Type:</b> {html.escape(str(product_type))}<br>
                <b>Colour:</b> {html.escape(str(colour))}<br>
                <b>Department:</b> {html.escape(str(department))}<br>
                <b>Group:</b> {html.escape(str(group))}<br>
                <b>Section:</b> {html.escape(str(section))}
            </div>
            {score_html}
        </div>
        """
